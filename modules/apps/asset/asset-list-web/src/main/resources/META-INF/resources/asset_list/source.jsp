...
>
	...
	<aui:select helpMessage="changing-this-setting-will-reset-all-fragments-mapping-for-this-collection" label="item-type" name="TypeSettingsProperties--anyAssetType--" title="item-type">
	...
	%>
		...
		<div class='asset-subtype <%= (assetSelectedClassTypeIds.length < 1) ? StringPool.BLANK : "hide" %>' id="<portlet:namespace /><%= className %>Options">
			<aui:select helpMessage="changing-this-setting-will-reset-all-fragments-mapping-for-this-collection" label='<%= LanguageUtil.get(request, "item-subtype") %>' name='<%= "TypeSettingsProperties--anyClassType" + className + "--" %>'>
				<aui:option label='<%= StringPool.DASH + LanguageUtil.get(request, "not-selected") + StringPool.DASH %>' selected="<%= editAssetListDisplayContext.isNoAssetTypeSelected() %>" value="" />
