...
export default function buildObjectDefinition({
	...
	// The object definition endpoint replaces the layouts it is given, so the
	// layout rides along with the definition rather than in a second request.

	if (includeObjectLayout && config.isNonRepeatableGroupsEnabled) {
		const objectLayout = buildObjectLayout({children, erc, label});
...
function getRepeatableGroups(
	...
	return flattenGroups(children).filter((child) =>
