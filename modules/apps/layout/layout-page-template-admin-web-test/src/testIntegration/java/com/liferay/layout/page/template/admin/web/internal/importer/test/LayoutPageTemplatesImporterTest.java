...
public class LayoutPageTemplatesImporterTest {
	...
	public void testImportLayoutPageTemplateEntry() throws Exception {
		...
		List<LayoutsImporterResultEntry> layoutImporterResultEntries = null;

		ServiceContextThreadLocal.pushServiceContext(_serviceContext2);

		try {
			layoutImporterResultEntries = _layoutsImporter.importFile(
		...
		Assert.assertNotNull(layoutImporterResultEntries);

		Assert.assertEquals(
			layoutImporterResultEntries.toString(), 1,
			layoutImporterResultEntries.size());

		LayoutsImporterResultEntry layoutsImportEntry =
			layoutImporterResultEntries.get(0);

		Assert.assertEquals(
			LayoutsImporterResultEntry.Status.IMPORTED,
			layoutsImportEntry.getStatus());

		String layoutPageTemplateEntryKey = StringUtil.toLowerCase(
			layoutsImportEntry.getName());
