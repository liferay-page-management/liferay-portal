...
public class ObjectUtil {
	...
	public static Map<String, List<Map<String, Object>>>
		getLayoutElementMapsListMap(long companyId, Group group) {
		...
		if (!GetterUtil.getBoolean(PropsUtil.get("feature.flag.LPS-150277")) ||
			hideInputFragments(companyId, group)) {
	...
	public static Boolean hideInputFragments(long companyId, Group group) {
		...
		for (ObjectDefinition objectDefinition : objectDefinitions) {
			if (_hasPermissions(group, objectDefinition)) {
				return false;
			}
		}
	...
	private static boolean _hasPermissions(
		...
		try {
			...
			return controlPanelEntry.hasAccessPermission(
				PermissionThreadLocal.getPermissionChecker(), group, portlet);
		}
