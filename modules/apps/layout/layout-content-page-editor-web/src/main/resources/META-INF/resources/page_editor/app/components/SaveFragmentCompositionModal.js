...
import validateFragmentComposition from '../thunks/validateFragmentComposition';
...
const SaveFragmentCompositionModal = ({itemId, onCloseModal}) => {
	...
	const getCompositionData = useCallback(
		...
		[
			...
			thumbnail.fileEntryId,
		]
	);

	const runValidation = useCallback(() => {
		return dispatch(validateFragmentComposition(getCompositionData()))
			.then((response) => {
				if (!isMounted()) {
					return 0;
				}

				const count = response.invalidFragmentsCount || 0;
	...
	}, [dispatch, getCompositionData, isMounted]);
	...
	const handleSave = () => {
		setLoading(true);
		dispatch(addFragmentComposition(getCompositionData()))
			.then(() => {
				onClose();
			})
			.catch(() => {
			...
			<InvisibleFieldset disabled={loading}>
				...
				>
					...
					<ClayInput
						autoFocus
						id={nameInputId}
						maxLength={config.fragmentCompositionNameMaxLength}
					...
					<ClayInput.Group>
						<ClayInput.GroupItem shrink>
							<ClayButton
								...
								onClick={() =>
									openImageSelector(handleThumbnailSelected)
								}
								size="sm"
					...
					<ClayInput.Group className="input-group-stacked-sm-down">
						<ClayInput.GroupItem className="mr-4" shrink>
							<ClayCheckbox
								...
								onChange={(event) =>
									setSaveInlineContent(event.target.checked)
								}
							/>
						</ClayInput.GroupItem>
	...
	return (
		<ClayModal
			...
			observer={observer}
			size={showErrorView ? 'md' : 'lg'}
			status={showErrorView ? 'danger' : undefined}
		>
			<ClayModal.Header
				closeButtonAriaLabel={Liferay.Language.get('close')}
			>
				{showErrorView
					? Liferay.Language.get('save-composition-not-allowed')
					: Liferay.Language.get('save-as-fragment')}
			</ClayModal.Header>

			<ClayModal.Body scrollable>
				{showErrorView ? renderErrorBody() : renderFormBody()}
			</ClayModal.Body>
			...
			<ClayModal.Footer
				last={
					<ClayButton.Group spaced>
						...
						{showErrorView ? (
							<ClayButton displayType="danger" onClick={onClose}>
