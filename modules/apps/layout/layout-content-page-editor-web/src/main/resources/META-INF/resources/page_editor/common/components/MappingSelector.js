...
function MappingSelector({fieldType, mappedItem, onMappingSelect}) {
	...
	useEffect(() => {
		const content = pageContents.find(
			(pageContent) =>
				pageContent.classNameId === selectedItem.classNameId &&
				pageContent.classPK === selectedItem.classPK
		);

		const type = selectedItem?.type || content?.name;
		const subtype = selectedItem?.subtype || content?.subtype;
