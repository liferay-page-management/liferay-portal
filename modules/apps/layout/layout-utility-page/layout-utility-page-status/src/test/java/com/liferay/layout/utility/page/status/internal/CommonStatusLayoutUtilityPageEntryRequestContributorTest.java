...
public class CommonStatusLayoutUtilityPageEntryRequestContributorTest {
	...
	public void testAddParametersWithDefaultVirtualHostAndWithoutCurrentURL() {
		VirtualHost mockVirtualHost = _getMockVirtualHost(
			0, RandomTestUtil.randomString());

		_mockPortal(null, mockVirtualHost.getHostname());

		_mockVirtualHostLocalService(mockVirtualHost);
	...
	public void testAddParametersWithVirtualHostWithoutLayoutsAndWithoutCurrentURL()
		...
		VirtualHost mockVirtualHost = _getMockVirtualHostWithoutLayouts();
	...
	public void testAddParametersWithVirtualHostWithPrivateLayoutAndWithoutCurrentURL()
		...
		Layout mockLayout = _getMockLayout();

		VirtualHost mockVirtualHost = _getMockVirtualHostWithLayouts(
			groupId, null, mockLayout);

		_mockPortal(null, mockVirtualHost.getHostname());

		_assertAttributesAndParameters(
			null, String.valueOf(groupId),
			String.valueOf(mockLayout.getLayoutId()));
	}
	...
	private Group _getMockGroup(long groupId) {
	...
	private LayoutSet _getMockLayoutSet(Group group) throws PortalException {
	...
	private VirtualHost _getMockVirtualHost(long layoutSetId, String name) {
	...
	private VirtualHost _getMockVirtualHostWithLayouts(
		...
		Group mockGroup = _getMockGroup(groupId);

		LayoutSet mockLayoutSet = _getMockLayoutSet(mockGroup);

		_mockLayoutLocalService(mockGroup, publicLayout, privateLayout);

		_mockLayoutSetLocalService(mockLayoutSet, mockVirtualHost);

		_mockVirtualHostLocalService(mockVirtualHost);

		return mockVirtualHost;
	}

	private VirtualHost _getMockVirtualHostWithoutLayouts()
		throws PortalException {

		Group mockGroup = _getMockGroup(RandomTestUtil.randomLong());

		LayoutSet mockLayoutSet = _getMockLayoutSet(mockGroup);

		_mockLayoutLocalService(mockGroup, null, null);
