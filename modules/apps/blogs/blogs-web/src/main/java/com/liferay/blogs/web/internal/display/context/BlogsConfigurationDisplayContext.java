...
public class BlogsConfigurationDisplayContext {
	...
	public String getBackURL() {
		...
		return PortletURLBuilder.create(
			requestBackedPortletURLFactory.createControlPanelRenderURL(
				BlogsPortletKeys.BLOGS_ADMIN, _themeDisplay.getScopeGroup(), 0,
				0)
		).buildString();
	}
	...
	public VerticalNavItemList getNotificationsVerticalNavItemList() {
		return VerticalNavItemListBuilder.add(
			...
			_getVerticalNavItemUnsafeConsumer("entry-added-email")
		).add(
			_getVerticalNavItemUnsafeConsumer("entry-updated-email")
		).build();
	}
