...
public class UpgradeFragmentEntryVersion extends UpgradeProcess {
	...
	protected void insertIntoFragmentEntryVersion() throws Exception {
		try (Statement s = connection.createStatement()) {
			...
			sb.append(" and FragmentEntry.fragmentEntryId not in (select ");
			sb.append("FragmentEntryVersion.fragmentEntryId from ");
			sb.append("FragmentEntryVersion)");
	...
	protected void upgradeFragmentEntryVersionCounter() throws Exception {
		runSQL(
			StringBundler.concat(
				...
				"', max(fragmentEntryId) from FragmentEntry where status = ",
